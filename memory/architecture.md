...
## Обзор потока данных
```
[Браузер] → /api/wb-proxy (Vercel Serverless) → [WB Statistics API]
                                                        ↓
[Браузер] ← JSON ← /api/wb-proxy ← ← ← ← ← ← [WB Statistics API]
```
Фронтенд НИКОГДА не обращается к WB API напрямую. Всегда через серверлесс-прокси.
Content API НЕ используется (не нужен токен с правами «Контент»).
Список товаров строится из данных статистики (продажи + финотчёт + остатки).
