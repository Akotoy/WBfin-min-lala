      ...
      <Card className="border-none shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex gap-2">
            <Input placeholder="Поиск по артикулу..." className="w-64" aria-label="Поиск товаров по артикулу" />
          </div>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {products.map((product) => (
                <TableRow key={product.nmId}>
                  ...
                  <TableCell>
                    <Button variant="ghost" size="sm" onClick={() => handleUpdatePrice(product.nmId, product.price + 100)} aria-label={`Увеличить цену на 100 рублей для ${product.title}`}>
                      +100 ₽
                    </Button>
                  </TableCell>
                </TableRow>
